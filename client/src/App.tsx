...
import DocumentConversionHub from "./features/companies/DocumentConversionHub";
    ...
    <main className="min-h-screen bg-slate-950 px-4 grid place-items-center text-slate-100">
      <section className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-8 shadow-2xl">
        <div className="mb-8">
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight text-white">ProAxis</h1>
        ...
        <form onSubmit={signIn} className="space-y-4">
          ...
          <button disabled={busy} className="w-full rounded-lg bg-indigo-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60">
            {busy ? "Signing in…" : "Sign in to ProAxis"}
          </button>
        </form>
      </section>
    </main>
...
function FullScreen({ title, text, signOut = false }: { title: string; text: string; signOut?: boolean }) {
  ...
  return <main className="min-h-screen bg-slate-950 px-4 grid place-items-center text-slate-200"><section className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-8 text-center"><p className="text-xs font-bold uppercase tracking-[0.2em] text-indigo-400">ProAxis</p><h1 className="mt-2 text-xl font-black text-white">{title}</h1><p className="mt-3 text-sm text-slate-400">{text}</p>{signOutError && <p className="mt-4 rounded-lg border border-rose-800 bg-rose-950/50 p-2 text-xs text-rose-200">{signOutError}</p>}{signOut && <button type="button" disabled={signingOut} onClick={handleSignOut} className="mt-6 rounded-lg border border-slate-600 px-4 py-2 text-sm font-semibold hover:bg-slate-800 disabled:opacity-60">{signingOut ? "Signing out…" : "Sign out"}</button>}</section></main>;
}
...
export default function App() {
  ...
  if (access.loading) return <FullScreen title="Loading ProAxis" text="Loading your secure workspace…" />;
  ...
  const navigation = [
    ...
    { id: "documents", label: "Document Conversion", permission: "documents.view" },
