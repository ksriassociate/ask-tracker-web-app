...
export const CustomersPage = () => {
  ...
  const saveCustomer = async () => {
    const payload = {
      ...
      gst_number: currentCustomer.gst_number, // Included in payload
    };
            ...
            <table className="min-w-full">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="text-left px-4 py-2">GST Number</th>
              ...
              <tbody>
                {customers.map((cust) => (
                  <tr key={cust.id} className="border-t hover:bg-gray-50 transition">
                    <td className="px-4 py-2 font-medium">{cust.company_name}</td>
                    <td className="px-4 py-2">{cust.gst_number || "-"}</td>
      ...
      {modalOpen && (
        ...
        >
          <div className="space-y-3">
            ...
            />
            {/* Added GST Input Field */}
            <input
              className="border p-2 w-full rounded"
              placeholder="GST Number"
              value={currentCustomer.gst_number}
              onChange={(e) => setCurrentCustomer({ ...currentCustomer, gst_number: e.target.value })}
            />
            <input
              className="border p-2 w-full rounded"
